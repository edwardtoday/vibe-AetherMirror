  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      const flowNoise = (x: number, y: number, t: number) => {
        return {
          x: Math.sin(0.0015 * x + t * 0.7) * 60,
          y: Math.cos(0.0012 * y + t * 0.6) * 60,
        }
      }
          ...
          for (let px = 0; px < dw; px += 2) {
            ...
            if (motion > 0.25) {
              for (let k = 0; k < 10; k++) {
                ...
                velocities[id * 2] += (Math.random() - 0.5) * 120 * motion
                velocities[id * 2 + 1] += (Math.random() - 0.5) * 120 * motion
              }
            }
          }
        }
